...
import type {
  ...
  PengeluaranPerusahaanPage,
} from "@/lib/pengeluaran-perusahaan";
import type { Unit } from "@/lib/unit";
import { cn } from "@/lib/utils";
import { toast } from "@/lib/toast-manager";
import type { ApiResponse, Pagination } from "@/types/api";
import { PengeluaranApprovalDialog } from "./pengeluaran-approval-dialog";
import { PengeluaranFormDialog } from "./pengeluaran-form-dialog";
...
export function PengeluaranTable({
  ...
  initialPage: PengeluaranPerusahaanPage;
  unitOptions: Unit[];
  ...
  const [editingItem, setEditingItem] = useState<PengeluaranPerusahaan | null>(
    null,
  );
  const [deletingItem, setDeletingItem] =
    useState<PengeluaranPerusahaan | null>(null);
  ...
  async function fetchPage(overrides: {
    ...
    try {
      const res = await fetch(
        `/api/proxy/finance/pengeluaran-perusahaan?${query.toString()}`,
        { cache: "no-store" },
      );
      const body: ApiResponse<PengeluaranPerusahaan[]> = await res.json();
      if (!res.ok || !body.success) {
        throw new Error(body.message || "Gagal memuat daftar pengeluaran");
      }
      setItems(body.data ?? []);
  ...
  function openEdit(item: PengeluaranPerusahaan) {
    setEditingItem(item);
    setDialogOpen(true);
  }

  function handleSaved(saved: PengeluaranPerusahaan) {
    setItems((prev) =>
  ...
  function handleApprovalDone(updated: PengeluaranPerusahaan) {
    setItems((prev) =>
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(
        `/api/proxy/finance/pengeluaran-perusahaan/${deletingItem.id}`,
        { method: "DELETE" },
      );
      const body: ApiResponse<null> = await res.json();
      if (!res.ok || !body.success) {
        toast.error(body.message || "Gagal menghapus pengeluaran");
        return;
      }
    ...
    <>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold">Pengeluaran Perusahaan</h1>
      ...
      {items.length > 0 && pagination && totalItems > items.length && (
        <p className="text-muted-foreground text-sm">
          Menampilkan {items.length} dari {totalItems} pengeluaran. Gunakan
      ...
      >
        <DialogContent className="max-w-sm">
          ...
          <DialogBody>
            <p className="text-sm">
              Yakin ingin menghapus pengajuan pengeluaran senilai{" "}
