...
// Helper filter dropdown lokal — pola yang sama dengan tabel master data lain
// (disalin per file, bukan diimpor dari sumber bersama).
...
export function RevenueUnitReport({
  initialRows,
  initialTahun,
  unitOptions,
}: {
  ...
  async function fetchReport(overrides: {
    ...
    try {
      const res = await fetch(
        `/api/proxy/business/revenue-unit?${query.toString()}`,
        { cache: "no-store" },
      );
  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold">Revenue Unit</h1>
          <p className="text-muted-foreground text-sm">
            Target vs realisasi revenue per unit (realisasi berbasis kas dari
            pencairan tagihan).
          </p>
        </div>
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                ...
                >
                  Belum ada data revenue unit untuk tahun {tahun}.
                </td>
              </tr>
            ) : (
              rows.map((row, index) => (
