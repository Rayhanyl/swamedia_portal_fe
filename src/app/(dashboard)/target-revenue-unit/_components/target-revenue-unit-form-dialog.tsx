...
import type { TargetRevenueUnit } from "@/lib/target-revenue-unit";
...
function formFromTarget(item: TargetRevenueUnit): FormState {
  return {
    id: item.id,
    unitId: item.unitId,
...
export function TargetRevenueUnitFormDialog({
  open,
  onOpenChange,
  editing,
  unitOptions,
  ...
  editing: TargetRevenueUnit | null;
  unitOptions: Unit[];
  onSaved: (saved: TargetRevenueUnit) => void;
}) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch(
        isEdit
          ? `/api/proxy/business/target-revenue-unit/${form.id}`
          : "/api/proxy/business/target-revenue-unit",
        {
      ...
      const resBody: ApiResponse<TargetRevenueUnit> = await res.json();
      if (!res.ok || !resBody.success || !resBody.data) {
        toast.error(resBody.message || "Gagal menyimpan target revenue unit");
        return;
      }
      onSaved(resBody.data);
      toast.success(
        isEdit
          ? "Target revenue unit berhasil diperbarui"
          : "Target revenue unit berhasil ditambahkan",
      );
      onOpenChange(false);
    } catch {
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="text-primary-foreground">
            {form.id === null
              ? "Tambah Target Revenue Unit"
              : "Edit Target Revenue Unit"}
          </DialogTitle>
        </DialogHeader>
