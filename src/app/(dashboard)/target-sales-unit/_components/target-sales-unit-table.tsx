...
import type {
  TargetSalesUnit,
  TargetSalesUnitPage,
} from "@/lib/target-sales-unit";
import { toast } from "@/lib/toast-manager";
import type { Unit } from "@/lib/unit";
import { cn, formatRupiah } from "@/lib/utils";
import type { ApiResponse, Pagination } from "@/types/api";
import { TargetSalesUnitFormDialog } from "./target-sales-unit-form-dialog";
...
export function TargetSalesUnitTable({
  initialPage,
  unitOptions,
}: {
  initialPage: TargetSalesUnitPage;
  unitOptions: Unit[];
}) {
  ...
  const [editingItem, setEditingItem] = useState<TargetSalesUnit | null>(null);
  const [deletingItem, setDeletingItem] = useState<TargetSalesUnit | null>(
    null,
  );
  const [deleting, setDeleting] = useState(false);

  // Referensi tak dipakai untuk debounce di sini (tak ada input teks), tapi
  // disiapkan bila kelak ada pencarian bebas.
  ...
  async function fetchPage(overrides: {
    ...
    try {
      const res = await fetch(
        `/api/proxy/business/target-sales-unit?${query.toString()}`,
        { cache: "no-store" },
      );
      const body: ApiResponse<TargetSalesUnit[]> = await res.json();
      if (reqId !== fetchRef.current) return;
      if (!res.ok || !body.success) {
        throw new Error(body.message || "Gagal memuat target sales unit");
      }
      setItems(body.data ?? []);
      setPagination(body.meta.pagination ?? null);
    } catch (err) {
      toast.error(
        err instanceof Error ? err.message : "Gagal memuat target sales unit",
      );
    } finally {
  ...
  function openEdit(item: TargetSalesUnit) {
    setEditingItem(item);
    setDialogOpen(true);
  }

  function handleSaved(saved: TargetSalesUnit) {
    setItems((prev) =>
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(
        `/api/proxy/business/target-sales-unit/${deletingItem.id}`,
        { method: "DELETE" },
      );
      const body: ApiResponse<null> = await res.json();
      if (!res.ok || !body.success) {
        toast.error(body.message || "Gagal menghapus target sales unit");
        return;
      }
      setItems((prev) => prev.filter((it) => it.id !== deletingItem.id));
      toast.success("Target sales unit berhasil dihapus");
    ...
    <>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold">Target Sales Unit</h1>
