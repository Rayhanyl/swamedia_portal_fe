...
// Kembaran RevenueUnitTwReport untuk sisi sales — bentuk data & tabel identik
// (LaporanUnitTwRow), beda endpoint proxy (/business/sales-matrix/tw) dan
// konteks realisasi (deal, bukan kas). File terpisah mengikuti konvensi
// aplikasi yang tidak berbagi komponen lintas-route.
...
export function PencapaianSalesUnitReport({
  initialRows,
  initialTahun,
  initialTriwulan,
  unitOptions,
  ...
  async function fetchReport(overrides: {
    ...
    try {
      const res = await fetch(
        `/api/proxy/business/sales-matrix/tw?${query.toString()}`,
        { cache: "no-store" },
      );
      const body: ApiResponse<LaporanUnitTwRow[]> = await res.json();
      if (!res.ok || !body.success) {
        throw new Error(body.message || "Gagal memuat pencapaian sales unit");
      }
      setRows(body.data ?? []);
    } catch (err) {
      toast.error(
        err instanceof Error
          ? err.message
          : "Gagal memuat pencapaian sales unit",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold">Pencapaian Sales Unit</h1>
          <p className="text-muted-foreground text-sm">
            Target vs realisasi sales (deal) per unit untuk satu triwulan
            terpilih.
          </p>
        </div>
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                ...
                >
                  Belum ada data pencapaian sales untuk triwulan ini.
                </td>
              </tr>
            ) : (
              rows.map((row, index) => (
