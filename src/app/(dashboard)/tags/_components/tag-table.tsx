...
import type { Tag, TagPage } from "@/lib/tags";
...
export function TagTable({
  ...
  async function fetchPage(overrides: {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/proxy/master/tags?${query.toString()}`, {
        cache: "no-store",
      });
      const body: ApiResponse<Tag[]> = await res.json();
      if (!res.ok || !body.success) {
        throw new Error(body.message || "Gagal memuat daftar tag");
      }
      setItems(body.data ?? []);
      setPagination(body.meta.pagination ?? null);
    } catch (err) {
      toast.error(
        err instanceof Error ? err.message : "Gagal memuat daftar tag",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/proxy/master/tags/${deletingTag.id}`, {
        method: "DELETE",
      });
